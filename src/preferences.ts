...
export const DEFAULT_SETTINGS: BooxSettings = {
  encryptedSession: null, region: "eur", account: null, syncFolder: "BOOX", intervalMinutes: 30,
...
export function restorePreferences(raw: unknown): { settings: BooxSettings; session: Session | null; removeLegacy: boolean } {
  ...
  if (!Number.isInteger(settings.intervalMinutes) || settings.intervalMinutes < 0 || settings.intervalMinutes > 1440) settings.intervalMinutes = 30;
  ...
  if (!settings.encryptedSession && typeof data.token === "string" && data.token && ["eur", "push"].includes(String(data.region))) {
